...
def main():
    ...
    # 気象庁・環境省から海外IPとして拒否されないためのブラウザ偽装ヘッダー
    headers = {
    ...
    try:
        # 観測所テーブル取得
        table_bytes = fetch_url("https://www.jma.go.jp/bosai/amedas/const/amedastable.json", headers)
        stn_table = json.loads(table_bytes.decode('utf-8'))

        # 最新時刻取得
        time_bytes = fetch_url("https://www.jma.go.jp/bosai/amedas/data/latest_time.json", headers)
        ...
        print(f"気象庁データ取得成功 (観測所: {len(stn_table)}件, 時刻: {latest_time_str})")

    except Exception as e:
        print(f"【重大エラー】気象庁データの取得に失敗しました: {e}")
        # ★データが取得できなかった場合は、空のJSONを書き出さずにエラー終了させる
        sys.exit(1)
    ...
    print(f"成功: {len(output_stations)}件のデータを wbgt_data.json に正常保存しました。")
