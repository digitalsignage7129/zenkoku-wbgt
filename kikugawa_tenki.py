...
# 静岡県西部（菊川市が属する天気予報区分。右帯テンプレートと同じ根拠で採用）
AREA_CODE = "220040"
# 御前崎観測所（菊川市に最も近い気温予測拠点。右帯テンプレートと同じ根拠で採用）
# 田原本町版はweekly/短期の気温シリーズをareas[0]で決め打ちしていたが、
# 静岡県西部の気温シリーズには浜松・御前崎など複数地点が並ぶため、
# 決め打ちだと菊川市と関係のない地点の気温を拾ってしまう(実際に右帯開発時に
# 判明した問題と同じ)。そのため気温だけはTEMP_CODEで明示的に一致させる。
TEMP_CODE = "50551"
...
def find_area(areas, codes):
    ...
    見つからなければ先頭要素にフォールバックする(田原本町版の挙動を踏襲)。"""
    if not areas:
        return {}
    for code in codes:
        for a in areas:
...
def get_jma_weather():
    ...
    # JSON書き出し
    with open("kikugawa_tenki.json", "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)

    print("kikugawa_tenki.json を正常に生成しました:")
